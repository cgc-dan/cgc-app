...
class Menu extends lifecycle_1.Disposable {
    ...
    createMenu(items) {
        items.forEach((menuItem) => {
            ...
            if (menuItem.type === 'separator') {
                ...
                item = new menuitem_1.MenuItem(menuItem, menuItemOptions, this.closeSubMenu);
                if (this.options.enableMnemonics) {
                    const mnemonic = item.getMnemonic();
                    if (mnemonic && item.isEnabled()) {
                        let actionItems = [];
...
exports.Menu = Menu;
class Submenu extends menuitem_1.MenuItem {
...
class Separator extends menuitem_1.MenuItem {
